    ...
    <div className={styles.galleryContainer}>
      ...
      <div className={styles.thumbnailsContainer}>
        <label htmlFor="file-upload" className={styles.addThumbnail}>
          <span role="img" aria-label="Adicionar Foto">
            ➕
          </span>{' '}
          Adicionar Foto
        </label>
...
export default function ProviderPerfil({ userData = mockUserData }) {
  ...
  const handleUpdateProfile = useCallback(async () => {
    if (profileId) {
      try {
        ...
        const nestedProfile =
          meData.perfil_prestador || meData.perfil_cliente || {};
  ...
  useEffect(() => {
    ...
    if (
      providerAccount?.portfolio &&
      Array.isArray(providerAccount.portfolio)
    ) {
    ...
    if (file) {
      ...
      try {
        ...
        const finalUrl = getImageUrl(
          result.imagem || result.image || result.url,
        );

        setUserGalleryImages((prevImages) =>
          prevImages.map((item) =>
            item.id === tempId ? { id: result.id, url: finalUrl } : item,
          ),
        );
        setCurrentMainImage((current) =>
          current === tempUrl ? finalUrl : current,
        );
        ...
        setUserGalleryImages((prevImages) =>
          prevImages.filter((item) => item.id !== tempId),
        );
  ...
  const handleDeleteImage = async (id) => {
    ...
    try {
      ...
      if (
        currentMainImage ===
        userGalleryImages.find((item) => item.id === id)?.url
      ) {
  ...
  const handleCompleteService = async (id) => {
    if (
      window.confirm(
        'Deseja marcar este serviço como concluído? Isso enviará uma mensagem para o cliente.',
      )
    ) {
      try {
        ...
        if (link) {
          ...
          alert(
            'Serviço marcado como concluído! (Link do WhatsApp não recebido)',
          );
        }
      } catch (error) {
  ...
  const handleMarkNotRealized = async (id) => {
    if (
      window.confirm(
        'Tem certeza que deseja marcar este serviço como NÃO realizado? O cliente não poderá avaliar.',
      )
    ) {
      try {
  ...
  const notaMedia =
    providerAccount?.nota_media ||
    providerAccount?.estatisticas?.media_geral ||
    0;
    ...
    for (let i = 0; i < 5; i++) {
      stars.push(
        <span
          key={i}
          style={{
            color: i < fullStars ? '#ffc107' : '#e4e5e9',
            fontSize: '24px',
          }}
        >
          ★
        ...
        <div className={styles.perfil}>
          <div className={styles.imgEdit} onClick={handlePhotoEditClick}>
            <img
              src={
                getImageUrl(
                  providerAccount?.foto || providerAccount?.foto_perfil,
                ) ||
                userData.perfilImg ||
      ...
      <div className={styles.container}>
        ...
        <div className={styles.box}>
          ...
          <div className={styles.descricaoGrid}>
            ...
            <span>
              Disponibilidade:{' '}
              {providerAccount?.disponibilidade === true
                ? 'Disponivel 24h'
                : 'Indisponivel 24h'}
            </span>
            ...
            {providerAccount?.servicos_nao_realizados_cache > 0 && (
              <span style={{ color: 'red', fontWeight: 'bold' }}>
                ...
                {providerAccount.servicos_nao_realizados_cache}
              </span>
            )}
            ...
            <div className={styles.box2}>
              ...
              <textarea
                ...
                value={providerAccount?.biografia || ''}
                readOnly
              ></textarea>
            </div>
          </div>
          ...
          {solicitations.length === 0 ? (
            ...
            solicitations.map((sol) => (
              <div key={sol.id} className={styles.solicit}>
                <h5 className={styles.soli}>
                  {sol.cliente_nome || sol.cliente || 'Cliente'}
                </h5>
                <h5 className={styles.soli}>
                  {sol.data_clique
                    ? new Date(sol.data_clique).toLocaleDateString('pt-BR')
        ...
        <div className={`${styles.box} ${styles.avaliacoesBox}`}>
          ...
          >
            <div
              style={{ fontSize: '48px', fontWeight: 'bold', color: '#1a06c9' }}
            >
              {Number(notaMedia).toFixed(1)}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex' }}>{renderStars(notaMedia)}</div>
              <span
