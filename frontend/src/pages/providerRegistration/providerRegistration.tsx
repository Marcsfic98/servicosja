...
const formatCategoryToKey = (name: string) => {
  return name
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
export default function ProviderRegistration() {
  ...
  // Services
  const { registerProvider, loading } = useProviderServices();
  const { categories, fetchCategories, loadingCategories } =
    useCategoryServices();
  ...
  const getErrorMessage = (fieldName) => {
    if (
      ...
      Array.isArray(formErrors[fieldName]) &&
  ...
  const handleChangeSetDataProvider = useCallback(
    (e) => {
  ...
  const handleSubmit = (e) => {
    ...
    registerProvider(formDataProvider)
      .then((result) => {
        setAuthData(result);
    ...
    <div className={styles.userRegistrationContainer}>
      <div className={styles.registrationForm}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input
            type="password"
            value={formDataProvider.password || ''}
            placeholder="Senha"
            onChange={handleChangeSetDataProvider}
            name="password"
            required
          ...
          <input
            type="password"
            value={formDataProvider.password2 || ''}
      ...
      <div className={styles.registrationImage}>
        <img
          src="/img/registration/registrationProvider.jpeg"
          alt="Imagem de cadastro de usuário"
        />
      </div>
    </div>
  );
